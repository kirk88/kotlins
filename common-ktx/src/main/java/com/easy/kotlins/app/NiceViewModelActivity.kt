 ...
 */
abstract class NiceViewModelActivity<VM: ViewModel>(layoutResId: Int) : NiceActivity(layoutResId), EventObserver, ViewModelOwner<VM> {
    ...
    final override fun onEventChanged(event: Event) {
        for (fragment in supportFragmentManager.fragments) {
            if ((fragment as? NiceViewModelFragment<*>)?.onInterceptViewModelEvent(event) == true) {
                return
            }
        }
